...
import { FlaskConical, Wrench, Droplets, ShieldCheck, Beaker, Hammer } from "lucide-react";

/**
 * PURELLA WATER — WEBSITE (React + Tailwind, hash router)
 * - Logo centers in header (no tagline). Logo is Home button.
 * - Pages: Home, Advanced Filtration, Softening, Saltless Conditioners, Well Water, Ultraviolet, Process, Water Score
 * - Water Score page fetches live data from /api/waterscore (EPA Envirofacts/SDWIS)
 */
...
function CtaPrimary({ href, children }: { href: string; children: React.ReactNode }) {
  return <a href={href} className="inline-flex items-center rounded-2xl px-5 py-3 text-base font-semibold shadow-sm bg-cyan-600 text-white hover:bg-cyan-700 transition">{children}</a>;
}
        ...
        <div className="max-w-7xl mx-auto px-4 py-10 md:py-14 grid md:grid-cols-[1.4fr_.9fr] gap-6 items-stretch">
          <div className="rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-white/5">
            <div className="aspect-[16/9] md:aspect-[21/10] relative">
              <img src="/images/hero-water.jpg" alt="Family kitchen with Purella water" className="absolute inset-0 w-full h-full object-cover" />
    ...
    <main className="max-w-7xl mx-auto px-4 py-16">
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        ...
        <div className="rounded-3xl border p-6">
          ...
          <div className="mt-3 bg-slate-100 border rounded-2xl overflow-hidden">
...
function WellWaterPage() {
  return (
    <main className="max-w-7xl mx-auto px-4 py-16">
      ...
      <p className="mt-3 text-slate-700 max-w-3xl">Target iron, sulfur, manganese, and sediment with the right media stack — aeration, oxidation, and polishing based on your well test.</p>
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        <div className="rounded-3xl border p-6">
...
function WaterScorePage() {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-16">
      <SectionTitle>Water Score</SectionTitle>
      ...
      <div className="mt-10 rounded-3xl bg-slate-50 border p-6">
        <div className="font-bold">Important</div>
        <p className="text-sm text-slate-700 mt-1">
          ZIP lookups are estimates. Results can vary by neighborhood and treatment updates. Purella confirms everything with a free in‑home test and designs a system matched to your water.
        </p>
      </div>
    </main>
  );
...
export default function App() {
  ...
  const NavLink = ({ label, target }: { label: string; target: Page }) => (
    <a
      onClick={() => { setPage(target); window.location.hash = target; }}
    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b">
        <div className="max-w-7xl mx-auto px-4 py-2">
          ...
          <nav className="mt-2 flex items-center justify-center gap-1 md:gap-3">
          ...
          <div className="mt-2 flex items-center justify-center gap-3">
            ...
            <a onClick={() => { setPage("home"); window.location.hash = "home"; }} className="inline-flex items-center rounded-2xl px-4 py-2 text-sm md:text-base font-semibold shadow-sm bg-cyan-600 text-white hover:bg-cyan-700 transition cursor-pointer">Free Water Test</a>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-slate-700">
              ...
              <li><a onClick={() => { window.location.hash = "waterscore"; }} className="cursor-pointer">Water Score</a></li>
          ...
          <div>
            ...
            <a onClick={() => { window.location.hash = "home"; }} className="mt-3 inline-flex items-center rounded-2xl px-4 py-2 text-sm font-semibold shadow-sm bg-cyan-600 text-white hover:bg-cyan-700 transition cursor-pointer">Start My Free Test</a>
          </div>
        </div>
